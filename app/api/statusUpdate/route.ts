...
async function detectCollection(donationId: string): Promise<string> {
  ...
  const requestDoc = await requestsCollection.findOne({ _id: new ObjectId(donationId) });
  ...
  const donationDoc = await donationsCollection.findOne({ _id: new ObjectId(donationId) });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!ObjectId.isValid(donationId)) {
      return NextResponse.json({ error: 'Invalid donationId format' }, { status: 400 });
    }
    ...
    console.log(`Updating ${collectionName} with _id:`, donationId, 'to status:', nextStatus);
    ...
    if (result.matchedCount === 0) {
      const donation = await collection.findOne({ _id: new ObjectId(donationId) });
      console.log(`Status in DB for donationId ${donationId} in ${collectionName} is:`, donation?.status);
    }
