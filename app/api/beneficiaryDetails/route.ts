...
export async function GET(request) {
  try {
    ...
    const beneficiary = await collection.findOne({ email: email, role: 'beneficiary' });

    if (!beneficiary) {
      return new Response(JSON.stringify({ error: "Beneficiary not found or role mismatch" }), {
        status: 404,
      });
    }
...
export async function POST(request) {
  try {
    ...
    const { email, poc_name, poc_phone, agency, halal_certification, hygiene_certification, role } = body;
    ...
    return new Response(JSON.stringify({ message: "Beneficiary updated successfully" }), {
      status: 200,
    });
  } catch (error) {
